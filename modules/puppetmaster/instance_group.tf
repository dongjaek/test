...
data "template_file" "puppetmaster_dns" {
  template = "${file("~/infrastructure/test/resources/puppetmaster_dns.sh")}"
}
  ...
  "${
    merge(
      map(
        ...
        "script-puppetmaster_dns", "${data.template_file.puppetmaster_dns.template}",
